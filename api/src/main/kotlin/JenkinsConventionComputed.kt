...
public class JenkinsConventionComputed(private val extension: JenkinsConventionExtension) {
    ...
    private val githubUrl: Provider<URI> = extension.gitHub.map { url ->
    ...
    private val isPipelinePlugin: Provider<Boolean> =
    ...
    private val isBuildPlugin: Provider<Boolean> =
        extension.pluginId.zip(extension.pluginCategories) { id, categories ->
            id.contains("build") || id.contains("gradle") || id.contains("maven") || id.contains("ant") || categories.any { it == "build" }
        }

    private val isScmPlugin: Provider<Boolean> = extension.pluginId.zip(extension.pluginCategories) { id, categories ->
